...
class UserRepository:
    ...
    def create(self, user):
        ...
        if len(user.username) < 3 or re.match("^[a-z]+$", user.username) is None:
            raise Exception(
                f"Username must be at least 3 digits long and contain only letters"
            )

        if len(user.password) < 8 or not re.search("^[a-z]", user.password):
            raise Exception(
                f"Password must be at least 8 digits long and contain at least one number"
            )
